...
/* ==== TEMPORARY feedback attachment read — REMOVED in the same session ==== */
/* Read-only, CRON_SECRET-gated like the dump above. Returns one feedback attachment so a
   reported PDF can actually be looked at while fixing what produced it. Deleted in the same
   session; if you are reading this in main, something went wrong and it should be removed. */
...
/* ==== TEMPORARY feedback reply endpoint — REMOVED in the same session ==== */
/* Applies replies to feedback records and notifies each submitter — the same thing the admin
   screen's POST /api/feedback/{id} does, reached with the CRON_SECRET header instead of an
   SWA session cookie. Read the reply text from the body; never invent one. Deleted again in
   the same session. */
app.http('tmp-feedback-reply', {
  methods: ['POST'],
  authLevel: 'anonymous',
  route: 'tmp/feedback-reply',
  handler: async (request, context) => {
    const secret = process.env.CRON_SECRET || '';
    const given = request.headers.get('x-bcc-cron-secret') || '';
    if (!secret || given !== secret) return { status: 401, jsonBody: { ok: false, error: 'bad or missing cron secret' } };
    try {
      ...
      if (!items.length) return { status: 400, jsonBody: { ok: false, error: 'items[] required' } };
      const c = container();
      const out = [];
      for (const it of items) {
        const id = String(it && it.id || '');
        const note = String(it && it.note || '').trim().slice(0, 2000);
        if (!/^bcc-feedback-[A-Za-z0-9-]{1,64}$/.test(id) || !note) { out.push({ id, ok: false, why: 'bad id or empty note' }); continue; }
        const doc = await c.item(id, BCC_TENANT_ID).read().then(r => r.resource)
          .catch(e => { if (e && (e.code === 404 || e.statusCode === 404)) return null; throw e; });
        if (!doc || doc.docType !== 'feedback') { out.push({ id, ok: false, why: 'not found' }); continue; }
        const wasResolved = doc.status === 'resolved';
        const now = new Date().toISOString();
        doc.status = 'resolved';
        doc.reviewedBy = 'lyle@bluecollarcoach.us';
        doc.resolutionNote = note;
        doc.resolutionBy = 'lyle@bluecollarcoach.us';
        doc.resolutionAt = now;
        doc.updatedAt = now;
        await c.items.upsert(doc);
        // Notify the submitter with the reply, exactly as the admin path does.
        let notified = null;
        if (doc.userUpn) {
          notified = await notifyUser(c, doc.userUpn, {
            title: wasResolved ? '💬 Reply to your feedback' : '✅ Your feedback was addressed',
            body: note, url: safeNotifyPath(doc.page), tag: 'fbdone-' + doc.id
          });
        }
        out.push({ id, ok: true, notified: !!notified, upn: doc.userUpn || null });
      }
      return { jsonBody: { ok: true, results: out } };
    } catch (e) {
      return { status: 500, jsonBody: { ok: false, error: String(e && e.message || e) } };
    }
  }
