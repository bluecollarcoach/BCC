...
app.http('cron-reminders', {
  ...
  handler: async (request, context) => {
    ...
    try {
      ...
      /* 5c) A client whose nightly QuickBooks sync has been silently failing — the exact
         gap a 2026-09-12 log review found: Patriot Masonry Co. had gone 18+ straight days
         with "no periods synced" (alternating Intuit 400s and 429 throttling) and nobody
         had been told, because the only place this showed was a "Last sync" column on the
         Management dashboard nobody was staring at. lastSyncAt only advances on a
         SUCCESSFUL sync (see cron-qbo-sync) — lastSyncAttemptAt advances every night
         regardless of outcome, so it is the wrong field to alert on; lastSyncAt is the
         right one because it answers "have this client's numbers actually moved." Skipped
         for disabled companies (deliberately off) and privateToUpn-locked ones (the firm's
         own books, or another owner-only company — not this alert's business). Re-fires
         every STALE_SYNC_DAYS while it stays stale, on a bucket derived from the outage's
         AGE, so it does not go silent after the first alert the way a plain once-ever
         marker would. Reuses feedbackNotifyUpns() — the same "who owns the firm's
         operational health" list feedback lands on. */
      const STALE_SYNC_DAYS = 3;
      let staleSyncCount = 0;
      try {
        const { resources: allComps } = await c.items.query({
          query: 'SELECT c.realmId, c.companyName, c.enabled, c.privateToUpn, c.lastSyncAt, c.lastSyncError, c.connectedAt FROM c WHERE c.tenantId=@t AND c.docType="qbo-company"',
          parameters: [{ name: '@t', value: BCC_TENANT_ID }]
        }).fetchAll();
        const staleCutoff = now - STALE_SYNC_DAYS * REM_DAY_MS;
        for (const co of allComps) {
          if (co.enabled === false) continue;
          if (String(co.privateToUpn || '').trim()) continue;
          const lastGood = co.lastSyncAt ? Date.parse(co.lastSyncAt) : (co.connectedAt ? Date.parse(co.connectedAt) : NaN);
          if (isNaN(lastGood) || lastGood > staleCutoff) continue;
          const bucket = Math.floor((now - lastGood) / (STALE_SYNC_DAYS * REM_DAY_MS));
          const marker = 'stalesync:' + co.realmId + ':' + bucket;
          if (sent[marker]) continue;
          staleSyncCount++;
          const staleDays = Math.floor((now - lastGood) / REM_DAY_MS);
          const title = '⚠️ ' + (co.companyName || co.realmId) + ' hasn’t synced in ' + staleDays + ' day' + (staleDays === 1 ? '' : 's');
          const bodyText = (co.lastSyncError ? String(co.lastSyncError).slice(0, 160) : 'The nightly QuickBooks sync has not completed successfully.') + ' Check Management dashboard → reconnect if needed.';
          for (const owner of feedbackNotifyUpns()) {
            try {
              const stored = await notifyUser(c, owner, { title, body: bodyText, url: '/bookkeeping.html', tag: marker });
              if (stored) sent[marker] = now;
            } catch (_) {}
            try { await emailNotifyUser(owner, title, bodyText, '/bookkeeping.html'); } catch (_) {}
          }
        }
      } catch (e) {
        context.error && context.error('cron-reminders: stale-sync scan failed', e);
      }
      ...
      return { jsonBody: { ok: true, scanned: docs.length, due: dueCount, pushed: okCount, pruned: deadCount,
        ...
        staleSyncAlerts: staleSyncCount } };
    } catch (err) {
...
app.http('qbo-company-update', {
  ...
  handler: withAccessLog(async (request, context) => {
    ...
    try {
      ...
      // still mutate a company they can't see (e.g. plant a clientAppUrl link that the
      // owner then sees rendered by our own trusted chrome).
      ...
      const body = await request.json().catch(() => ({}));
      const me = String(p.userDetails || p.userId || '').toLowerCase();
      // Validate BEFORE any write so a bad request can never half-apply.
      // Over-length is rejected, not silently truncated — a mangled 500-char URL that
      // looks saved is worse than a clear error.
      let appUrl;
      if (typeof body.clientAppUrl === 'string') {
        const u = body.clientAppUrl.trim();
        if (u.length > 500) return { status: 400, jsonBody: { error: 'clientAppUrl is too long (max 500 characters)' } };
        if (u && !/^https?:\/\//i.test(u)) return { status: 400, jsonBody: { error: 'clientAppUrl must start with http:// or https://' } };
        appUrl = u;
      }
      // Apply the patch to a freshly-read doc, then write it back guarded by that read's
      // ETag. This doc is ALSO rewritten by qboAccessForCompany whenever Intuit rotates
      // the refresh token, so a blind full-doc upsert here could write back a stale,
      // already-invalidated token and silently kill the client's QuickBooks connection.
      // On a 412 (someone else wrote first) we re-read and re-apply, never clobber.
      const applyPatch = (d) => {
        ...
        if (body.privateToUpn !== undefined) {
          const cur = String(d.privateToUpn || '').toLowerCase();
          ...
          if (want) d.privateToUpn = me; else delete d.privateToUpn;
        }
        // Marks the firm's OWN books — the CRM customer import reads from this company.
        if (typeof body.isOwnBooks === 'boolean') {
          if (String(d.privateToUpn || '').toLowerCase() && String(d.privateToUpn).toLowerCase() !== me) return { status: 403, jsonBody: { error: 'not your company' } };
          d.isOwnBooks = body.isOwnBooks;
        }
        if (typeof body.enabled === 'boolean') d.enabled = body.enabled;
        if (Array.isArray(body.allowedUserUpns)) d.allowedUserUpns = body.allowedUserUpns.map(s => String(s).toLowerCase()).filter(Boolean);
        ...
        // anchor href, so anything else was already rejected above.
        if (appUrl !== undefined) d.clientAppUrl = appUrl;
      ...
      for (let attempt = 0; attempt < 3 && !saved; attempt++) {
        ...
        const rejected = applyPatch(cur);
        if (rejected) return rejected;
        try {
          const opts = cur._etag ? { accessCondition: { type: 'IfMatch', condition: cur._etag } } : undefined;
          const res = await c.items.upsert(cur, opts);
          saved = res.resource || cur;
        } catch (e) {
          if (e && (e.code === 412 || e.statusCode === 412)) { context.log('qbo-company-update etag conflict, retrying'); continue; }
          throw e;
        }
      }
      if (!saved) return { status: 409, jsonBody: { error: 'this company was being updated elsewhere — please try again' } };
      return { jsonBody: { ok: true, realmId, enabled: saved.enabled !== false, allowedUserUpns: saved.allowedUserUpns || [], privateToUpn: saved.privateToUpn || null, isOwnBooks: !!saved.isOwnBooks, clientAppUrl: saved.clientAppUrl || '' } };
    } catch (e) {
 ...
 * summarized by Customer — the QBO side of the job-costing / WIP schedule.
 */
app.http('qbo-job-costs', {
  methods: ['GET'], authLevel: 'anonymous', route: 'integrations/qbo/companies/{realmId}/job-costs',
  handler: withAccessLog(async (request, context) => {
    try {
...
function firmCfDefaults() {
  return {
    ...
    billingLines: {},          // key -> { include, monthly, customerName, itemName, confidence, months }
                                // confidence: 'confirmed' (default) | 'potential' — kept OUT of the
                                // cash projection until the owner flips it. months: null = ongoing,
                                // or a count of forward months (capped at FIRM_CF_MONTHS) after which
                                // this line stops contributing — a fixed-term engagement, not a raise.
      ...
      if (body.billingLines && typeof body.billingLines === 'object') {
        ...
        for (const k of Object.keys(bsrc)) {
          ...
          bl[String(k).slice(0, 300)] = {
            ...
            confidence: (l.confidence === 'potential') ? 'potential' : 'confirmed',
            // Clamped to the forecast horizon — a duration longer than the window shown is
            // indistinguishable from ongoing, and a negative one would count backwards.
            months: (l.months === null || l.months === undefined || l.months === '') ? null
              : Math.max(0, Math.min(FIRM_CF_MONTHS, Math.round(firmCfNumber(l.months))))
          };
        }
